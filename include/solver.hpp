...
namespace ratio
{
  ...
  private:
    ...
    std::unordered_map<const linspire::constraint *, std::vector<std::reference_wrapper<resolver>>> lin_cnst_to_resolvers; // mapping from linear constraints to the resolvers using them..
  };
} // namespace ratio
