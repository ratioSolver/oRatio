  ...
  export class SolverGraph extends Component<solver.Solver, HTMLDivElement> implements solver.SolverListener {
    ...
    override mounted(): void {
      ...
      this.simulation = d3.forceSimulation(this.nodes)
        .force('link', d3.forceLink<GraphNode, GraphLink>(this.edges).id(d => d.id).distance(100))
      ...
      // Append a rectangle to each node group
      node.append('rect')
        .attr('width', 100)
        .attr('height', 30)
        .attr('x', -50) // Center rectangle relative to the node 'circle'
        .attr('y', -15) // Vertically center rectangle
        .attr('rx', 5) // Rounded corners
        .attr('ry', 5)
        .attr('fill', '#69b3a2');

      // Append a text element to each node group
      node.append('text')
        .text(d => d.label)
