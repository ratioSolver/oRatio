...
namespace ratio
{
    ...
    void graph::set_flaw_state(flaw &f, utils::lbool state, bool resetting) noexcept
    {
        if (f.state != state)
        {
            ...
            if (state == utils::True && std::none_of(f.get_resolvers().begin(), f.get_resolvers().end(), [](const auto &resolver)
                                                     { return resolver.get().state == utils::True; }))
            {
    ...
    void graph::expand_flaw(flaw &f, bool remove)
    {
        ...
        assert(std::none_of(f.get_resolvers().begin(), f.get_resolvers().end(), [](const auto &resolver)
                            { return resolver.get().state == utils::False; })); // all the resolvers should be feasible..
    ...
    bool graph::is_deferrable(flaw &f)
    {
        ...
        while (!stk.empty())
        {
            ...
            for (const auto &support : c_f->get_supports())
                stk.push(&support.get().get_flaw()); // schedule to be checked..
        }
        // if stack empties, all reachable flaws were deferrable
        return true;
    }

    flaw::flaw(graph &gr, std::vector<std::reference_wrapper<resolver>> &&causes, const bool &exclusive) : gr(gr), causes(causes), exclusive(exclusive)
    {
