...
namespace ratio::solver
{
    ...
    {
        assert(get_solver().get_sat()->value(get_phi()) != semitone::False);
        assert(get_solver().get_sat()->value(get_solver().atom_properties.at(&atm).sigma) != semitone::False);
        if (get_solver().get_sat()->value(get_solver().atom_properties.at(&atm).sigma) == semitone::Undefined) // we check if the atom can unify..
            ...
            { // we check for possible unifications (i.e. all the instances of the atom's type)..
                ...
                if (!t_flaw.is_expanded() ||                                                                       // the target flaw must hav been already expanded..
                    ...
                    get_solver().get_sat()->value(get_solver().atom_properties.at(&t_atm).sigma) == semitone::False || // the target atom is unified with some other atom..
                ...
                if (get_solver().get_sat()->value(eq_lit) == semitone::False)
                ...
                assert(get_solver().get_sat()->value(u_res->get_rho()) != semitone::False);
    ...
    { // activating this resolver activates the fact..
        if (!get_solver().get_sat()->new_clause({!get_rho(), semitone::lit(get_solver().atom_properties.at(&atm).sigma)}))
