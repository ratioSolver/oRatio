...
namespace ratio::solver
{
    ...
    semitone::rational h_1::get_estimated_cost(const resolver &r) const noexcept
    {
        ...
        semitone::rational est_cost;
#ifdef H_MAX
        est_cost = semitone::rational::NEGATIVE_INFINITY;
        for (const auto &f : r.get_preconditions())
            if (!f->is_expanded())
                return semitone::rational::POSITIVE_INFINITY;
            else // we compute the maximum of the flaws' estimated costs..
            {
                semitone::rational c = f->get_estimated_cost();
                if (c > est_cost)
                    est_cost = c;
            }
#endif
    ...
    void h_1::propagate_costs(flaw &f)
    {
        ...
        else
        {
            ...
            c_cost = chpst_res ? chpst_res->get_estimated_cost() : semitone::rational::POSITIVE_INFINITY;
        }
